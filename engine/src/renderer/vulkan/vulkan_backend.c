...
b8 vulkan_renderer_backend_begin_frame(renderer_backend *backend,
  ...
  if (context.recreating_swapchain) {
    ...
    if (!vulkan_result_is_success(result)) {
      OERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (1) failed: '%s'", vulkan_result_string(result, TRUE));
      return FALSE;
    }
  ...
  if (context.framebuffer_size_generation != context.framebuffer_size_last_generation) {
    VkResult result = vkDeviceWaitIdle(device->logical_device);
    if (!vulkan_result_is_success(result)) {
      OERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (2) failed: '%s'", vulkan_result_string(result, TRUE));
      return FALSE;
    }
  ...
  // Wait for the execution of the current frame to complete. fence being free will let this move on.
  ...
  // Acquire next image from swapchain, send the semaphore that should be signaled when it's complete
  // Sempahore will later be waited on by the queue to ensure availability
...
b8 vulkan_renderer_backend_end_frame(renderer_backend *backend,
  ...
  if (result != VK_SUCCESS) {
    OERROR("vkQueueSubmit failed with result: %s", vulkan_result_string(result, TRUE));
	   return FALSE;
  }
...
i32 find_memory_index(u32 type_filter, u32 property_flags) {
 ...
 void create_command_buffers(renderer_backend* backend) {
   if (!context.graphics_command_buffers) {
     ...
     for (u32 i = 0; i < context.swapchain.image_count; ++i) {
       ozero_memory(&context.graphics_command_buffers[i], sizeof(vulkan_command_buffer));
     }
   }
   for (u32 i = 0; i < context.swapchain.image_count; ++i) {
     if (context.graphics_command_buffers[i].handle) {
...
b8 recreate_swapchain(renderer_backend* backend) {
    ...
    if (context.framebuffer_width == 0 || context.framebuffer_height == 0) {
        ODEBUG("recreate_swapchain called when window is < 1 in a dimension. Booting.");
        return FALSE;
    }
