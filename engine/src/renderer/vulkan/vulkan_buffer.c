...
b8 vulkan_buffer_create(vulkan_context *context, u64 size,
  ...
  ozero_memory(out_buffer, sizeof(vulkan_buffer));
  ...
  if (out_buffer->memory_index == -1) {
    OERROR("Unable to create vulkan buffer because the required memory type "
           "index was not found.");
    return false;
  }
  ...
  if (result != VK_SUCCESS) {
    OERROR("Unable to create vulkan buffer because the required memory "
...
b8 vulkan_buffer_resize(vulkan_context *context, u64 new_size,
  ...
  if (result != VK_SUCCESS) {
    OERROR("Unable to resize vulkan buffer because the required memory "
           "allocation failed. Error: %i",
           result);
    return false;
  }
...
void vulkan_buffer_load_data(vulkan_context *context, vulkan_buffer *buffer,
  ...
  VK_CHECK(vkMapMemory(context->device.logical_device, buffer->memory, offset,
                       size, flags, &data_ptr));
  ocopy_memory(data_ptr, data, size);
  vkUnmapMemory(context->device.logical_device, buffer->memory);
}
