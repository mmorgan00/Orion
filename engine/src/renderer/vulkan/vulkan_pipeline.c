...
b8 vulkan_graphics_pipeline_create(
  ...
  rasterizer_create_info.polygonMode =
      is_wireframe ? VK_POLYGON_MODE_LINE
                   : VK_POLYGON_MODE_FILL; // can't flip flop like in OpenGL
  ...
  ozero_memory(&color_blend_attachment_state,
  ...
  if (vulkan_result_is_success(result)) {
    ODEBUG("Graphics pipeline created!");
    return true;
  }

  OERROR("vkCreateGraphicsPipelines failed with %s.",
