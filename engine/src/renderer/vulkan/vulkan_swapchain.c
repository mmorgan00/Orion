...
b8 vulkan_swapchain_acquire_next_image_index(
  ...
  // Window out of date, recreate swapchain. Can happen on a window resize
...
void vulkan_swapchain_present(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        ...
        OFATAL("Failed to present swap chain image!");
    }
}
...
void create(vulkan_context *context, u32 width, u32 height,
  ...
  // Start with this as it HAS to exist by Vulkan spec
  ...
  for (u32 i = 0; i < context->device.swapchain_support.present_mode_count;
    ...
    // Look for mailbox - uses most recent, throws out old ones. Ideal for games
  ...
  swapchain_extent.width = OCLAMP(swapchain_extent.width, min.width, max.width);
  swapchain_extent.height = OCLAMP(swapchain_extent.height, min.height, max.height);
...
// Swapchain create info
    ...
    if (!swapchain->images) {
        swapchain->images = (VkImage*)oallocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views) {
        swapchain->views = (VkImageView*)oallocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    ...
    if (!vulkan_device_detect_depth_format(&context->device)) {
        ...
        OFATAL("Failed to find a supported format!");
    }
