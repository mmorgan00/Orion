...
b8 vulkan_object_shader_create(vulkan_context *context,
                               struct vulkan_object_shader *out_shader) {
  ...
  for (u32 i = 0; i < OBJECT_SHADER_STAGE_COUNT; i++) {
    if (!create_shader_module(context, BUILTIN_SHADER_NAME_OBJECT,
                              ...
                              out_shader->stages)) {
      OERROR("Unable to create %s shader module for '%s'.", stage_type_strs[i],
  ...
  // Descriptor pool for things like view/proj matrices
  ...
  VK_CHECK(vkCreateDescriptorPool(context->device.logical_device,
    ...
    ozero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
  ...
  ozero_memory(stage_create_infos, sizeof(stage_create_infos));
  for (u32 i = 0; i < OBJECT_SHADER_STAGE_COUNT; i++) {
  ...
  if (!vulkan_graphics_pipeline_create(
    ...
    OERROR("Failed to load graphics pipeline for object shader.");
    return false;
  }

  // Now that pipeline is created, create the descriptor buffer
  if (!vulkan_buffer_create(context, sizeof(global_uniform_object),
    ...
    OERROR("Vulkan buffer creation failed for object shader");
    return false;
  }

  // ALlocate descriptor layouts
  ...
  // Create the object uniform buffer.
    if (!vulkan_buffer_create(
        ...
        OERROR("Material instance buffer creation failed for shader.");
        return false;
    }
...
vulkan_buffer_destroy(context, &shader->object_uniform_buffer);
  ...
  vkDestroyDescriptorPool(logical_device, shader->global_descriptor_pool,
                          context->allocator);
...
void vulkan_object_shader_update_object(vulkan_context* context, struct vulkan_object_shader* shader, geometry_render_data render_data) {
    ...
    // Descriptors aka MATERIAL DATA
    vulkan_object_shader_object_state* object_state = &shader->object_states[render_data.object_id];
    ...
    ozero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
    u32 descriptor_count = 0;
    u32 descriptor_index = 0;

    // Descriptor 0 is the Uniform buffer
    u32 range = sizeof(object_uniform_object);
    u64 offset = sizeof(object_uniform_object) * render_data.object_id;
    object_uniform_object obo;

    // TODO: Load from an object spec, this is hard coded for now to show *something*
    static f32 accumulator = 0.0f;
    accumulator += context->frame_delta_time;
    f32 s = (osin(accumulator) + 1.0f) / 2.0f;  // scale from -1, 1 to 0, 1
...
b8 vulkan_object_shader_acquire_resources(vulkan_context* context, struct vulkan_object_shader* shader, u32* out_object_id) {
    ...
    *out_object_id = shader->object_uniform_buffer_index;
    ...
    for (u32 i = 0; i < VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT; ++i) {
        for (u32 j = 0; j < 3; ++j) {
            object_state->descriptor_states[i].generations[j] = INVALID_ID; // mark them as 'needing to generate'
        }
    }
    ...
    if (result != VK_SUCCESS) {
        OERROR("Error allocating descriptor sets in shader!");
        return false;
    }
...
void vulkan_object_shader_release_resources(vulkan_context* context, struct vulkan_object_shader* shader, u32 object_id) {
    ...
    if (result != VK_SUCCESS) {
        OERROR("Error freeing object shader descriptor sets!");
    }
