...
b8 vulkan_object_shader_create(vulkan_context *context,
  ...
  ozero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) *
...
void vulkan_object_shader_destroy(vulkan_context *context,
  ...
  vkDestroyDescriptorPool(logical_device, shader->object_descriptor_pool,
                          context->allocator);
...
void vulkan_object_shader_update_object(vulkan_context *context,
  ...
  vulkan_object_shader_object_state *object_state =
      &shader->object_states[render_data.object_id];
  ...
  ozero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) *
                                      VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
  ...
  // TODO: Load from an object spec, this is hard coded for now to show
  // *something*
...
b8 vulkan_object_shader_acquire_resources(vulkan_context *context,
  ...
  *out_object_id = shader->object_uniform_buffer_index;
  ...
  for (u32 i = 0; i < VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT; ++i) {
    for (u32 j = 0; j < 3; ++j) {
      object_state->descriptor_states[i].generations[j] =
          INVALID_ID; // mark them as 'needing to generate'
    }
  }
