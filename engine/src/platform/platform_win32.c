...
b8 platform_startup(platform_state *plat_state, const char *application_name,
  ...
  wc.lpszClassName = "orion_window_class";
  ...
  HWND handle =
      CreateWindowExA(window_ex_style, "orion_window_class", application_name,
  ...
  // show Window
  b32 should_activate =
      1; // TODO: If the window should not accept input, set false
...
// free
void platform_free(void *block, b8 aligned) { return (free(block)); }
...
LRESULT CALLBACK win32_process_message(HWND hwnd, u32 msg, WPARAM w_param,
  ...
  case WM_SIZE: {
    ...
    // Fire the event. The application layer should pick this up, but not handle
    // it as it shouldn be visible to other parts of the application.
