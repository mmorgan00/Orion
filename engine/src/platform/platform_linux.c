...
b8 platform_startup(platform_state *plat_state, const char *application_name,
  ...
  if (xcb_connection_has_error(state->connection)) {
    OFATAL("Failed to connect to X server via XCB.");
    return false;
  }
  ...
  if (stream_result <= 0) {
    OFATAL("An error occurred when flusing the stream: %d", stream_result);
    return false;
  }
...
b8 platform_pump_messages(platform_state *plat_state) {
  ...
  while ((event = xcb_poll_for_event(state->connection))) {
    ...
    case XCB_CONFIGURE_NOTIFY: {
      // TODO: Resizing
      // Resizing - note that this is also triggered by moving the window, but
      // should be passed anyway since a change in the x/y could mean an
      // upper-left resize. The application layer can decide what to do with
      // this.
      xcb_configure_notify_event_t *configure_event =
          (xcb_configure_notify_event_t *)event;

      // Fire the event. The application layer should pick this up, but not
      // handle it as it shouldn be visible to other parts of the application.
